...
import { calculateStats, getFingerName } from '../typing/KeyboardEngine';
import soundEngine from '../typing/SoundEngine';
import { motion, AnimatePresence } from 'framer-motion';
import { MousePointer2, RefreshCw, Zap } from 'lucide-react';
...
export default function TypingPractice({ onBack }) {
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (soundEnabled) {
      soundEngine.init();
      if (currentKey === ' ') soundEngine.play('space');
      else if (currentKey === 'Backspace') soundEngine.play('backspace');
      else soundEngine.play('standard', currentKey !== text[currentIndex] ? 0.7 : 1.0);
    }
  ...
  }, [currentIndex, text, startTime, isFinished, currentLesson, setupTest, soundEnabled]);
  ...
  return (
    <div className="w-full flex flex-col items-center relative overflow-hidden" style={{ minHeight: '80vh', background: 'linear-gradient(to bottom, #050505 0%, #0a0a0a 100%)' }}>
      ...
      <div className="mt-6 flex items-center gap-4">
        ...
        >
          <Zap size={12} className={soundEnabled ? 'animate-pulse' : ''} />
